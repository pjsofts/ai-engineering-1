// repl.ts — Part 3: the agent can run commands.
//
// Still unchanged. Three tools have been added across two parts and this file
// has not needed a single line — the interface draws whatever the session
// reports, and it learns each new tool's name from the data.
//
// This is the high-water mark for danger. mycode can now read your files,
// rewrite them, and run arbitrary shell commands, with nobody asked. It is one
// `rm -rf` away from a very bad afternoon, which is precisely the feeling
// Part 4 exists to fix. Keep it in `sandbox/`.
//
// Run:  npm run part3
...
async function main(): Promise<void> {
  banner("v0.2 · part 3 — and it has a shell", `${MODEL} — read · write · edit · bash · glob · grep`);
