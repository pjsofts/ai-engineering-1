// harness.ts — Part 5: the loop becomes a RUNTIME that runs any agent. Two
// changes: it carries `current` (starting at triage) and hydrates with THAT
// agent's prompt + tools; and `handoff` is intercepted — the harness swaps the
// running agent and keeps the conversation. `current` is rebuilt
// deterministically on replay (the handoff is a consequence of a cached model
// decision), so this composes with Part 2's durability for free.
