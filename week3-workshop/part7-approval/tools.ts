// tools.ts — Part 5 additions on top of Part 3: the privileged tool
// (issueRefund) and the handoff tool. runTool gains issueRefund like any other
// side-effecting tool — but NOT handoff. The RUNTIME intercepts that one: it
// switches the running agent instead of executing anything.
...
export const TOOLS: Tool[] = [
  ...
  fn("issueRefund",
     ...
     { customer_id: { type: "string" }, charge_id: { type: "string" },
       amount_cents: { type: "number" } },
     ["customer_id", "charge_id", "amount_cents"]),
  fn("handoff",
     "Hand the conversation to a specialist agent when the task needs a " +
     "capability you don't have (e.g. issuing a refund -> billing).",
     { to: { type: "string", enum: ["billing"] }, reason: { type: "string" } },
     ["to", "reason"]),
];
...
export async function runTool(name: string, args: Record<string, any>): Promise<string> {
  switch (name) {
    ...
    case "issueRefund":  // IRREVERSIBLE — Part 7 puts a human in front of this
      return JSON.stringify({ refunded: true, customer_id: args.customer_id,
