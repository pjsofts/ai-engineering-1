...
interface PlanStep { id: string; agent: "billing" | "technical" | "sales"; objective: string; }
interface Finding { agent: string; findings: string; }
...
async function makePlan(task: string): Promise<PlanStep[]> {
  const r = await client.chat.completions.create({ model: MODEL,
    messages: [
      { role: "system", content:
...
async function synthesize(task: string, findings: Finding[]): Promise<string | null> {
  const joined = findings.map((f) => `[${f.agent}] ${f.findings}`).join("\n\n") || "(none)";
...
export async function supervisorWorkflow(wf: Workflow): Promise<string | null> {
  ...
  const plan: PlanStep[] = await wf.runStep("plan", () => makePlan(wf.input ?? ""));
  await wf.runStep("plan-emit", () => emit({ type: "plan.created",
    workflow: wf.id, steps: plan.map((s) => s.agent) }));
  ...
  // supervisor down.
  const settled = await Promise.allSettled(plan.map((s) =>
    wf.runStep(`subagent-${s.id}`, async (): Promise<Finding> => {
      emit({ type: "subagent.started", workflow: wf.id,
             agent: s.agent, objective: s.objective.slice(0, 60) });
  ...
  for (let i = 0; i < settled.length; i++) {
    ...
    if (outcome.status === "fulfilled") {
      ...
      await wf.runStep(`subagent-failed-${plan[i].id}`, () => emit({
        type: "subagent.failed", workflow: wf.id,
        agent: plan[i].agent, error: String(outcome.reason).slice(0, 80) }));
    }
  }
  // SYNTHESIZE from whatever survived. Partial results beat no results.
  const reply = await wf.runStep("synthesize", () => synthesize(wf.input ?? "", findings));
  await wf.runStep("completed", () => emit({ type: "workflow.completed",
    workflow: wf.id, output: (reply ?? "").slice(0, 60) }));
  wf.finish();
  return reply;
}
