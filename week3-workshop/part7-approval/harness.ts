// harness.ts — Part 7: the tool loop grows a THIRD branch. handoff is
// intercepted (Part 5); everything in NEEDS_APPROVAL goes through the
// approvalGate — which either returns a cached/recorded human decision or
// PARKS the workflow (status "suspended") and exits the process. approve.ts
// flips it back to "running"; the next launch replays straight to the gate.
...
import { NEEDS_APPROVAL, approvalGate } from "./approvals.js";
...
export async function agentWorkflow(wf: Workflow): Promise<string | null> {
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    for (const call of calls) {
      ...
      } else if (NEEDS_APPROVAL.has(call.function.name)) {
        const decision = await approvalGate(wf, call, args); // may throw Suspended
        result = decision.approved
          ? await execTool(wf, call)
          : JSON.stringify({ approved: false, message:
              "A human did NOT approve this action. Do not retry - tell " +
              "the customer it needs manual review." });
      } else {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (process.argv.includes("--resume-only")) process.exit(0); // just the recovery pass
  ...
  try {
    ...
    if (e instanceof Suspended)
      console.log(`${e.message} is waiting for a human. ` +
                  `Decide with: npx tsx approve.ts ${e.message} yes|no`);
    else throw e;
  }
}
